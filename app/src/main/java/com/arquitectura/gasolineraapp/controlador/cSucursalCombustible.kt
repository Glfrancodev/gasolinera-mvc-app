...
class cSucursalCombustible(private val activity: Activity) {
    ...
    private val modelo: ISucursalCombustible = ProxySucursalCombustible()
    ...
    private fun agregarSucursalCombustible() {
        ...
        val nuevo = mSucursalCombustible(
            idSucursal = sucursales[posSucursal].id,
            idCombustible = combustibles[posCombustible].id,
            cantidadBombas = cantidad,
            horaMedicion = obtenerFechaActual(),
            combustibleDisponible = litros
        )

        if (modelo.crear(activity, nuevo)) {
    ...
    private fun editarSucursalCombustible() {
        seleccion!!.apply {
            idSucursal = sucursales[vista.getSucursalSeleccionada()].id
            idCombustible = combustibles[vista.getCombustibleSeleccionado()].id
            cantidadBombas = vista.getCantidadBombas() ?: 0
            combustibleDisponible = vista.getLitrosDisponibles() ?: 0.0
        }

        if (modelo.actualizar(activity, seleccion!!)) {
    ...
    private fun eliminarSucursalCombustible(item: mSucursalCombustible) {
        if (modelo.eliminarPorId(activity, item.id)) {
